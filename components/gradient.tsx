...
export class NeatGradient implements NeatController {
  ...
  constructor(config: NeatConfig & { ref: HTMLCanvasElement; resolution?: number; seed?: number }) {
    ...
    const render = () => {
      ...
      meshes.forEach(mesh => {
        ...
        // @ts-ignore
        mesh.material.uniforms.u_color_pressure = {
          value: new THREE.Vector2(this._horizontalPressure, this._verticalPressure),
        };
        // @ts-ignore
  ...
  _buildMaterial(width: number, height: number) {
    ...
    const material = new THREE.ShaderMaterial({
      ...
      fragmentShader:
        buildUniforms() + buildColorFunctions() + buildNoise() + buildFragmentShader(),
    });
...
function updateCamera(camera: THREE.Camera, width: number, height: number) {
  ...
  const targetPlaneArea = ((areaViewPort / viewPortAreaRatio) * PLANE_WIDTH * PLANE_HEIGHT) / 1.5;
